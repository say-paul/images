...
func TestLVMVCreateMountpoint(t *testing.T) {
	...
	entity, err := vg.CreateMountpoint("/", 0, false)
	assert.NoError(err)
	rootlv := entity.(*LVMLogicalVolume)
	assert.Equal("rootlv", rootlv.Name)

	_, err = vg.CreateMountpoint("/home_test", 0, true)
	assert.NoError(err)

	entity, err = vg.CreateMountpoint("/home/test", 0, true)
	assert.NoError(err)
	...
	// Lets collide it
	for i := 0; i < 98; i++ {
		_, err = vg.CreateMountpoint("/home/test", 0, false)
		assert.NoError(err)
	}
